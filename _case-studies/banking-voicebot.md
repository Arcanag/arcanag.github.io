...
<strong>Role:</strong> Project Manager, end-to-end ownership of a 100K-call/day banking voicebot. <strong>Problem:</strong> Inherited broken metrics — containment was fiction (20K actual vs 8L target). <strong>Impact:</strong> Built a call resolution funnel from scratch, drove SLU accuracy from 70% to 91%, survived 15 critical incidents, and shipped 30 use cases across 5 delivery waves.
</div>
...
- **Fragile Phase 1 deployment** — A single live use case, working but brittle
- **Unreliable platform** — Flow builder, authentication, and analytics all had known reliability issues
- **Incomplete NLU model** — Only **74 intents**, many incorrectly tagged as out-of-scope
- **Zero technical artifacts** — Business requirement documents existed, but **no reusable flows, no technical specs, and no handover documentation**
...
- **Full retraining** — Genuinely new intents requiring fresh model architecture
- **Entity additions** — Existing intents needing new slot types for multi-product routing
- **Targeted edge-case testing** — Ambiguous UPI queries, grid value activation conflicts, and sequential card type probing demanded careful architectural overhauls, not just raw injections of more training data
...
We considered three options: a full SLU retrain, conversation flow redesign only, or the hybrid model. A complete SLU retrain would have taken **6+ months** — not including the preliminary research required in the air-gapped environment. The hybrid approach let us show immediate, incremental progress while laying the groundwork for larger model improvements.
